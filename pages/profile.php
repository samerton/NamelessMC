				...
				if($exists == true){
					...
					if($friends !== false){
						foreach($friends as $friend){
							echo '<span rel="tooltip" title="' . $user->IdToName($friend->friend_id) . '"><a href="profile.php?user=' .  $user->IdToName($friend->friend_id) . '"><img class="img-rounded" src="https://cravatar.eu/avatar/' . $user->IdToMCName($friend->friend_id) . '/40.png" /></a></span> ';
						}
					} else {
