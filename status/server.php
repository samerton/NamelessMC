...
class ServerStatus {
	public function serverPlay($server_ip, $server_port){
		$Info = false;
		$Query = null;
	...
	if( $Query !== null )
	{
		...
		if($Info['players']['online'] == 0) {
			...
			for ($row = 0; $row < $max; $row++) {
				echo '<span rel="tooltip" data-trigger="hover" data-original-title="' . htmlspecialchars($Info["players"]["sample"][$row]["name"]) . '"><a href="../profile.php?user=' . htmlspecialchars($Info["players"]["sample"][$row]["name"]) . '"><img src="https://cravatar.eu/avatar/' . htmlspecialchars($Info["players"]["sample"][$row]["name"]) . '/50.png" style="width: 40px; height: 40px; margin-bottom: 5px; margin-left: 5px; border-radius: 3px;" /></a></span>';
			}
			if(isset($extra)){
